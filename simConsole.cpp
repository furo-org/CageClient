...
int main(int argc, char* argv[]) {
  ...
  options.add_options()
    ...
    ("server,s", bo::value<std::string>(), "Server address and port (e.g. 127.0.0.1:54323");
  ...
  try {
    ...
    if (values.count("help")) {
      std::cout << "usage: simconsole [options] [console command to exec]" << std::endl;
  ...
  if (!ctx) {
    std::cerr << "Cannot create zcontext:" << zmq_strerror(zmq_errno()) << std::endl;
    exit(1);
  }
  if (!ctx->isValid()) {
    std::cerr << "Cannot create zcontext:" << zmq_strerror(zmq_errno()) << std::endl;
